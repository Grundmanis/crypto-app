...
export class CoinExchangeCronService {
  ...
  startJob(seconds: number) {
    ...
    const job = new CronJob(cronExpression, () => {
      this.logger.warn(`time (${seconds}) for job ${CoinExchangeCronService.name} to run!`);
    ...
    job.start();
    this.logger.warn(`job ${CoinExchangeCronService.name} added for each minute at ${seconds} seconds!`);
  }
